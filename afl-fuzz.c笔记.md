# **afl-fuzz.c笔记**
## **前言：**
本文适合对象：已经对afl的流程有一定了解，自己跑过afl的各个功能；具有一定C编程基础，浏览过afl源码或者某个模块的源码；  
笔记分为五个大块对afl-fuzz.c进行分析：文件引用、预备工作、fuzzing的整体结构、关键函数实现原理、main函数。
## **Ⅰ、文件引用**
### 【一】自定义头文件  
...
### 【五】fuzzing策略  
这部分跟 `mutation` 相关，会多写一点，将会分为两部分进行展开：**现有AFL的策略**和**论文相关的魔改AFL策略**
