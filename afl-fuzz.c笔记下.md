## **下篇序：**
上一篇已经将前三部分写完了，但是留有一些疑问点，所以下篇将分三部分：对上篇遗留问题的解决、关键函数实现原理介绍、main函数注释（逐行贴出代码块，因为main是整个alf的核心流程，所以需要详解，当然上篇讲到的在第五部分就会一笔掠过啦）  
最后会有附录，写有我看过实用的相关链接。
...
当时没找到，后来转过弯来，其实那不是变量也不是普通的函数，`EFF_ALEN`是一个宏定义的函数：
``` C++
...
``` C++
//此部分我写了下伪代码，宏定义虽然写代码的时候好用，有点像内联函数（个人感觉）但是不得不说全是大写字母有点难理解，所以我又给拆分成了通俗的代码，这部分跟上下文关系不大，看懂这部分，其他类似的也很好懂了。
type(_p) EFF_APOS(_p){
    // >> 是位移操作，把参数的二进制形式进行右移，每移动一位就减小2倍，所以之歌函数的意思就是，返回传入参数_p除以（2的EFF_MAP_SCALE2次方）。同理另一个方向就是左移，是放大2的幂的倍数。
    return (_p / 8);
...
/*
前两个的宏函数本质知道了，分别代表的什么意思呢？（个人觉得）  
APOS是指
REM是指
*/
type(_l) EFF_ALEN(_l){
    //这里的 !! 是一个两次否，目的是归一化（又是一个骚操作，这个作者写代码真的是净整些这种，主要是自己菜）
    //比如 r = !!a，如果a是整数0，则r=0，如果a是整数非0，则r=1。在a不是整数的情况下一般不这么用，这里都是默认为整数的，字符型转成ascii那不也是整数吗。
    return 
}
```
#### 2. bitflip
